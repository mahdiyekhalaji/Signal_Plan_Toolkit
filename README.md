...
`docs/index.html` is a self-contained page. Three things recompute in the
browser rather than being baked in: the corridor offsets (move the cycle or
progression speed and the time-space diagram redraws), HCM control delay and LOS
for all twelve movements at Yonge and Steeles, and a sweep of progression
efficiency and intersection delay against cycle length, which shows that the two
do not peak at the same cycle. Enable GitHub Pages on the
