...
class Intersection:
    ...
    def markings(self):
        ...
        for setback in (9.0, 25.0):
            # rotation 270 points +x (east), 90 points -x (west): the arrow
            # must point the way that approach travels
    ...
    def equipment(self):
        ...
        # one head per lane, on the far side of the intersection facing the
        # approach it governs
        for i in range(len(self.approaches["n"])):
            self.blk("SIG-3SEC", self.med_ns + (i + 0.5) * lw, hy, 180)
        for i in range(len(self.approaches["s"])):
            self.blk("SIG-3SEC", -(self.med_ns + (i + 0.5) * lw), -hy, 0)
        for i in range(len(self.approaches["e"])):
            self.blk("SIG-3SEC", hx, -(self.med_ew + (i + 0.5) * lw), 90)
        for i in range(len(self.approaches["w"])):
            self.blk("SIG-3SEC", -hx, self.med_ew + (i + 0.5) * lw, 270)

        for sx, sy in ((-1, 1), (1, 1), (1, -1), (-1, -1)):
            self.blk("SIG-PED", sx * (self.hw_ns + 2.0), sy * (self.hw_ew + 7.5),
                     0 if sy > 0 else 180)
            self.blk("SIG-PED", sx * (self.hw_ns + 7.5), sy * (self.hw_ew + 2.0),
                     90 if sx > 0 else 270)
            # one pushbutton per crossing, so each crosswalk has its own call
            self.blk("SIG-PB", sx * (self.hw_ns + 2.0), sy * (self.hw_ew + 4.5))
            self.blk("SIG-PB", sx * (self.hw_ns + 4.5), sy * (self.hw_ew + 2.0))
        ...
        for sx, sy in ((-1, 1), (1, 1), (1, -1), (-1, -1)):
            self.msp.add_blockref(
                "SIGN-POST", (sx * (self.hw_ns + 10.5), sy * (self.hw_ew + 9.5)),
